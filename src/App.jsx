...
const TokyoTrip = () => {
  ...
  const deleteDay = (dayIndex) => {
    ...
    setSchedule(newSchedule);
    if (activeDay > newSchedule.length) {
      setActiveDay(newSchedule.length || 1);
    }
  };
          ...
          <div className="overflow-x-auto no-scrollbar flex space-x-3 snap-x flex-1">
            {schedule.map((day) => (
              <div key={day.day} className="relative snap-center flex-shrink-0">
                ...
                {schedule.length > 1 && (
                  <button
                    onClick={() => deleteDay(day.day - 1)}
                    className="absolute -top-2 -right-2 p-1 bg-red-500 hover:bg-red-600 text-white rounded-full shadow-md transition-colors opacity-0 hover:opacity-100"
                    title="刪除此日程"
                  >
              ...
              <div className="flex-1 pb-6">
                <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition-shadow duration-300 relative overflow-hidden">
                  ...
                  <div className="flex items-start justify-between relative z-10">
                    ...
                    <button
                      onClick={() => deleteEvent(activeDay - 1, idx)}
